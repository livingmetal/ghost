...
Orkia/
  ...
  image-prompt.json             # 외부 이미지 생성 지시서용 고정/가변 프롬프트 프로필
...
오르키아 이미지 프롬프트는 이미지 생성 API를 직접 호출하지 않는다. 앱과 외부 작업자가 동일한 기준으로 프롬프트를 만들 수 있도록 `image-prompt.json`에 고정 속성, 참조 이미지, 금지 drift, 프레이밍/포즈/mood 문구만 둔다.
